...
${DIR}/../../environment/plaintext/start.sh "${PWD}/docker-compose.plaintext.cdb-table-repro-86616.yml"
...
log "Waiting 30s for connector to read existing data"
sleep 30

log "Verifying topic ORCLCDB.C__MYUSER.TIP: there should be 2 records"
set +e
timeout 60 docker exec connect kafka-avro-console-consumer -bootstrap-server broker:9092 --property schema.registry.url=http://schema-registry:8081 --topic ORCLCDB.C__MYUSER.TIP --from-beginning --max-messages 2 > /tmp/result.log  2>&1
set -e
cat /tmp/result.log
log "Check there is 2 snapshots events"
if [ $(grep -c "op_type\":{\"string\":\"R\"}" /tmp/result.log) -ne 2 ]
then
     logerror "Did not get expected results"
     exit 1
fi

# [2022-01-06 15:40:47,767] ERROR [cdc-oracle-source-cdb|task-1] WorkerSourceTask{id=cdc-oracle-source-cdb-1} Task threw an uncaught and unrecoverable exception. Task is being killed and will not recover until manually restarted (org.apache.kafka.connect.runtime.WorkerTask:206)
...
# 	at io.confluent.connect.avro.AvroData.validateSchemaValue(AvroData.java:1282)
# 	at io.confluent.connect.avro.AvroData.fromConnectData(AvroData.java:394)
